...
WELDER_STATES = ["Idle", "Moving CNC to Next CV Point", "Awaiting CV result",
                 "Awating Start Mark", "Move to Point", "Awaiting CNC to place",
                 "Awating Laser Sync"]
...
class MyPlugin(Plugin):
    ...
    def __init__(self, context):
        ...
        self.welder_status_sub = rospy.Subscriber('/welder/status', String, self.status_callback,
                queue_size=10)
        self.welder_time_sub = rospy.Subscriber('/welder/time', String, self.time_callback, queue_size=10)
        self.welder_progress_sub = rospy.Subscriber('/welder/progress', String, self.progress_callback,
                queue_size=10)
        self.image_sub = rospy.Subscriber("/laser_ctrl/img_feed/compressed", CompressedImage,
                self.image_callback, queue_size=10)
        self.laser_state_sub = rospy.Subscriber('/laser_ctrl/cmd', String, self.laser_state_clbk,
                queue_size=10)
    ...
    def progress_slot(self, data):
        blade_count = int(ros_data.data) # this will not work, ros_data not defined
